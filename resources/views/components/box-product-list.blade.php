...
<article class="bg-white overflow-hidden rounded-xl shadow-xl">
    <div class="flex flex-row justify-start items-stretch w-full">
        <div class="relative flex w-40 py-4 pl-2">
            <img src="{{ Storage::url($item->firstSku()->images->first()->url ?? 'products/no-image.png') }}" class="w-24 h-24" title="{{ Str::title($item->name) }}" alt="{{ Str::title($item->name) }}">
            ...
            <div class="absolute top-2 left-2">
                <div class="flex justify-center items-center bg-[#FF0000] rounded-lg drop-shadow-3xl px-2 py-0.5">
                    ...
                    @if ($promotion->type_promotion == 1)
                        <span class="text-[10px] font-bold text-white">-{{ number_format($promotion->discount_rate) }}%</span>
                    @else
                        <span class="text-[10px] font-bold text-white">SALE</span>
                    @endif
                </div>
            </div>
            @endif     
        ...
        <div class="flex flex-col justify-start items-stretch w-full mx-2 py-4">
            ...
            <div class="box-product-detail-rating">
                <ul>
                    ...
                    <li>
                        <i class='bx bxs-star level-rating {{ $item->rating >= 2 ? 'active' : 'no-active' }}'></i>
                    </li>
                    <li>
                        <i class='bx bxs-star level-rating {{ $item->rating >= 3 ? 'active' : 'no-active' }}'></i>
                    </li>
                    <li>
                        <i class='bx bxs-star level-rating {{ $item->rating >= 4 ? 'active' : 'no-active' }}'></i>
                    </li>
                    <li>
                        <i class='bx bxs-star level-rating {{ $item->rating == 5 ? 'active' : 'no-active' }}'></i>
                    </li>
                </ul>
                <p>{{ $item->rating }}</p>
            </div>
        ...
        <div class="flex flex-col justify-start items-stretch">
            <div class="grid grid-rows-2 h-full">
                <button type="button" onclick="showModalQuickViewsProduct('{{ $item->id }}')" class="box-product-btn-preview-list">
                    <svg class="w-6 h-auto" viewBox="0 0 20 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <button type="button" onclick="addToCartProduct('{{ $item->id }}')" class="box-product-btn-add-cart-list">
                    <svg class="w-6 h-auto md:w-6 md:h-6" viewBox="0 0 63 55" fill="none" xmlns="http://www.w3.org/2000/svg">
