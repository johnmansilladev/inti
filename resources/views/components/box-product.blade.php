@props(['item'])
<article class="box-product group">
    <div class="box-product-image">
        <a href="{{ route('product.index',$item) }}">
            <figure class="aspect-w-16 aspect-h-9">
                <img class="object-cover object-center" src="{{ Storage::url($item->firstSku()->images->first()->url) }}" alt="{{ Str::title($item->name) }}">
            </figure>
    ...
    <div class="flex flex-row bg-theme-gray rounded-b-3xl">
        <button type="button" onclick="showModalQuickViewsProduct('{{ $item->id }}')" class="box-product-btn-preview">
        ...
        <button type="button" onclick="addToCartProduct('{{ $item->id }}')" class="box-product-btn-add-cart"> 
            <svg class="mr-2 w-4 h-4 md:w-6 md:h-6" viewBox="0 0 63 55" fill="none" xmlns="http://www.w3.org/2000/svg">
