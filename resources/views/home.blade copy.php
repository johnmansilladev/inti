<x-app-layout>
                            ...
                            <a href="{{ $slider->redirect_to }}" class="{{ $slider->is_redirect == 0 ? 'pointer-events-none' : '' }}" target="{{ $slider->new_tab == 1 ? '_blank' : '_self' }}">
                                <div class="relative">
                                    ...
                                    <div class="absolute inset-0 h-full">
                                        <div class="max-w-screen-2xl mx-auto grid grid-cols-2 h-full">
                                            <div class="col-span-1 pl-16 pb-32 mt-auto">
                                                @if ($slider->title)
                                                    <h1 class="text-4xl text-white font-extrabold tracking-tight">{{ $slider->title }}</h1>
                                                @endif
                                                @if ($slider->content)
                                                    <p class="mt-6 text-white text-lg">{{ $slider->content }}⁣</p>   
                                                @endif
                                                @if ($slider->text_button)
                                                    <div class="mt-6">
                                                        <button type="button" class="flex items-center justify-center py-2 px-20 border border-transparent text-base font-bold rounded-md text-white uppercase bg-gradient-to-r from-theme-yellow to-theme-orange">{{ $slider->text_button }}</button>
                                                    </div>
                                                @endif
                                            </div>
                                        </div>
    ...
    <section>
        <div class="max-w-screen-2xl mx-auto my-10">
            <div class="max-w-4xl mx-auto">
                <h1 class="text-4xl font-extrabold text-theme-gray text-center uppercase">INTI DIESEL</h1>
                <p class="text-lg text-theme-gray text-center mt-4">Ya sea que esté buscando un software de diagnostico o requiera de soporte técnico, contamos con un amplio catálogo y ofrecemos atención las 24 horas desde donde esté.</p>
            </div>
        </div>
    </section>
    {{ $banners[0]->clustered->products }}
    {{-- @if (!is_null($banners[0]) && $banners[0]->clustered->products->count())
    <section style="background: linear-gradient(180deg, #FDC900 0%, #FDA200 100%);">
        <div class="py-20">
            <h1 class="text-4xl font-extrabold text-white text-center uppercase mb-4">{{ $banners[0]->title }}</h1>
            <div x-data="{swiper: null}" x-init="swiper = new Swiper($refs.container, {
                ...
                <div class="swiper max-w-screen-2xl mx-auto pt-10 pb-20 px-1" x-ref="container">
                    <ul class="swiper-wrapper">
                        @foreach ($banners[0]->clustered->products as $item)
                            <li class="swiper-slide">
    ...
    @endif --}}
    {{-- @if (!is_null($banners[1]) && $banners[1]->clustered->products->count())
    <section class="bg-theme-swhite">
        <div class="py-20">
            <h1 class="text-4xl font-extrabold text-theme-gray text-center uppercase">{{ $banners[1]->title }}</h1>
            <p class="max-w-4xl mx-auto text-lg text-theme-gray text-center mt-4">{{ $banners[1]->description }}⁣</p>
            <div x-data="{swiper: null}" x-init="swiper = new Swiper($refs.container, {
                loop: true,
                slidesPerView: 1,
                spaceBetween: 0,
                ...
                <div class="swiper max-w-screen-2xl mx-auto pt-10 pb-20 px-1" x-ref="container">
                    <ul class="swiper-wrapper">
                        @foreach ($banners[1]->clustered->products as $item)
                            <li class="swiper-slide">
                                <x-box-product :item="$item"/>
                            </li>
                        @endforeach
    ...
    <section>
        <div class="max-w-screen-xl mx-auto py-20">
            <h1 class="text-4xl font-extrabold text-theme-gray text-center uppercase mb-10">categorías</h1>
            <ul role="list" class="grid grid-cols-2 gap-12 sm:grid-cols-3 lg:grid-cols-4">
                @foreach ($categories as $category)
                    <li class="col-span-1">
                        <a href="{{ route('category.index', $category) }}" class="flex flex-col items-center justify-center w-full h-full bg-theme-yellow rounded-lg shadow-xl p-6 group hover:bg-gradient-to-r hover:from-theme-yellow hover:to-theme-orange hover:cursor-pointer transform transition duration-500 hover:scale-110">
    ...
    @endif
    {{-- @if (!is_null($banners[2]) && $banners[2]->clustered->products->count())
    <section class="bg-theme-swhite">
        <div class="max-w-screen-2xl mx-auto py-20">
            <h1 class="text-4xl font-extrabold text-theme-gray text-center uppercase">{{ $banners[2]->title }}</h1>
            <p class="max-w-4xl mx-auto text-lg text-center text-theme-gray mt-4 mb-10">{{ $banners[2]->description }}⁣⁣</p>
            <ul role="list" class="grid grid-cols-2 gap-12 sm:grid-cols-3 lg:grid-cols-5">
                @foreach ($banners[2]->clustered->products as $item)
                    <li class="">
                        <x-box-product :item="$item"/>
                    </li>
                @endforeach
            ...
            <div class="flex justify-center items-center mt-20">
                <button type="button" class="flex items-center justify-center py-1 px-20 border border-transparent text-base font-bold rounded-md text-white uppercase bg-gradient-to-r from-theme-yellow to-theme-orange">Ver Más</button>
            </div>
        </div>
    </section> 
    @endif --}}
    {{-- <section class="bg-theme-lwgray">
        <div class="max-w-screen-2xl mx-auto text-center pt-20 pb-12">
            <h1 class="text-4xl font-extrabold text-theme-lead text-center uppercase mb-12">COMPRA FÁCIL Y RÁPIDO</h1>
