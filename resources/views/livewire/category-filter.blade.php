            ...
            <div class="bg-white py-2 px-6 rounded-lg w-72">
              ...
              <div x-data="{ open: false }" class="mt-1 relative">
                ...
                <ul x-show="open" class="absolute z-10 mt-1 w-full bg-white shadow-lg max-h-68 rounded-md py-1 ring-1 ring-black ring-opacity-5 overflow-auto focus:outline-none text-sm" tabindex="-1" role="listbox" aria-labelledby="listbox-label" aria-activedescendant="listbox-option-1"  @click.outside="open = false" style="display: none">
                    <li wire:click="$set('sortFilter', 'OrderByRecommended')" class="text-theme-gray cursor-default select-none relative py-2 px-6 border-l-4 border-l-transparent hover:border-l-4 hover:border-l-theme-orange hover:bg-theme-lwgray hover:cursor-pointer {{ $sortFilter == 'OrderByRecommended' ? 'border-l-theme-orange bg-theme-lwgray' : '' }}" @click="open = false" id="listbox-option-0" role="option">
                      <span class="font-normal block truncate"> Recomendados </span>
                    </li>
                    <li wire:click="$set('sortFilter', 'OrderByNameASC')" class="text-theme-gray cursor-default select-none relative py-2 px-6 border-l-4 border-l-transparent hover:border-l-4 hover:border-l-theme-orange hover:bg-theme-lwgray hover:cursor-pointer {{ $sortFilter == 'OrderByNameASC' ? 'border-l-theme-orange bg-theme-lwgray' : '' }}" @click="open = false"  id="listbox-option-1" role="option">
                        <span class="font-normal block truncate"> Nombre A - Z </span>
                    </li>
                    <li wire:click="$set('sortFilter', 'OrderByNameDESC')" class="text-theme-gray cursor-default select-none relative py-2 px-6 border-l-4 border-l-transparent hover:border-l-4 hover:border-l-theme-orange hover:bg-theme-lwgray hover:cursor-pointer {{ $sortFilter == 'OrderByNameDESC' ? 'border-l-theme-orange bg-theme-lwgray' : '' }}" @click="open = false" id="listbox-option-2" role="option">
                        <span class="font-normal block truncate"> Nombre Z - A </span>
                    </li>
                    <li wire:click="$set('sortFilter', 'OrderByPriceASC')" class="text-theme-gray cursor-default select-none relative py-2 px-6 border-l-4 border-l-transparent hover:border-l-4 hover:border-l-theme-orange hover:bg-theme-lwgray hover:cursor-pointer {{ $sortFilter == 'OrderByPriceASC' ? 'border-l-theme-orange bg-theme-lwgray' : '' }}" @click="open = false" id="listbox-option-5" role="option">
                        <span class="font-normal block truncate"> Precio de menor a mayor</span>
                    </li>
                    <li wire:click="$set('sortFilter', 'OrderByPriceDESC')" class="text-theme-gray cursor-default select-none relative py-2 px-6 border-l-4 border-l-transparent hover:border-l-4 hover:border-l-theme-orange hover:bg-theme-lwgray hover:cursor-pointer {{ $sortFilter == 'OrderByPriceDESC' ? 'border-l-theme-orange bg-theme-lwgray' : '' }}" @click="open = false" id="listbox-option-6" role="option">
