        ...
        <div class="max-w-[70%] mx-auto py-14">
            <nav aria-label="Breadcrumb">
                <ol role="list" class="flex items-center space-x-4">
                    ...
                    <li>
                        <div class="flex items-center">
                            <a href="{{ route('shop',['shop_section' => 'category', 'shop_section_url' => $product->category->slug]) }}"
            ...
            <div class="py-4">
                <div class="flex">
                     <!-- Image gallery -->
                    <div x-data="{img_selected: @entangle('img_selected')}" class="w-3/5 px-4 py-10 flex flex-wrap border-r border-gray-200">
                        <!-- Image selector -->
                        <div class="w-1/6 h-[550px] overflow-y-auto scrollbar scrollbar-thumb-theme-yellow scrollbar-track-gray-100 scrollbar-theme">
                            <div class="grid grid-rows-4 gap-2" aria-orientation="horizontal" role="tablist">
                                @foreach ($sku_selected->images as $key => $image)
                                    <button wire:click="$set('img_selected','{{ Storage::url($image->url) }}')"
                                        class="relative bg-white min-h-80 aspect-w-1 aspect-h-1 rounded-md flex items-center justify-center text-sm font-medium uppercase text-gray-900 cursor-pointer hover:bg-gray-50 focus:outline-none focus:ring focus:ring-offset-4 focus:ring-opacity-50 px-[7px]" type="button">
                        ...
                        <div class="w-4/6 mx-auto">
                            <div>
                                <img :src="img_selected" class="w-full h-full object-center object-cover sm:rounded-lg">
                            </div>
                        </div>
                    </div>
                    <!-- Product info -->
                    <div class="w-2/5 pl-16">
                        <h1 class="text-2xl font-extrabold uppercase text-theme-gray">{{ $product->name }} - {{ $sku_selected->name }}</h1>
                            ...
                            <div class="bg-theme-lwgray rounded-lg p-6">
                                <div x-data="{ open: true }">
                                    ...
                                    <div class="prose prose-sm" id="disclosure-1" x-show="open">
                                        <ul role="list">
                                            <li>
                                                <span class="font-semibold text-theme-gray">Etiquetas:</span> Automotor
                                            </li>
                                            <li>
        ...
        <div class="max-w-[80%] mx-auto pb-12 pt-6">
            <div x-data="{ tab: 1 }">
                <ul class="grid grid-cols-3 gap-20">
                    <li @click="tab = 1" class="py-2 text-base text-center font-bold text-theme-gray rounded-lg hover:cursor-pointer" :class="tab == 1 ? 'bg-theme-yellow' : 'bg-theme-lwgray'">Información General</li>
                    <li @click="tab = 2" class="py-2 text-base text-center font-bold text-theme-gray rounded-lg hover:cursor-pointer" :class="tab == 2 ? 'bg-theme-yellow' : 'bg-theme-lwgray'">Actualización</li>
                    <li @click="tab = 3" class="py-2 text-base text-center font-bold text-theme-gray rounded-lg hover:cursor-pointer" :class="tab == 3 ? 'bg-theme-yellow' : 'bg-theme-lwgray'">Vehiculos Compatibles</li>
                </ul>
                <div class="mt-8">
